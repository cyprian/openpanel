...
Slack OAuth client ID for Slack integration. Required only when Slack integrations are enabled.
...
Slack OAuth client secret for Slack integration. Required only when Slack integrations are enabled.
...
**Example**:
```bash
SLACK_OAUTH_REDIRECT_URL=https://analytics.example.com/api/webhook/slack
```
...
Secret for signing Slack OAuth state parameter. Required only when Slack integrations are enabled.
