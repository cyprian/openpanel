...
| `list_ml_images` | List image artifact metadata for a run without embedding image bytes. |
| `list_ml_evaluation_rows` | List visual evaluation rows for a run without embedding image bytes. |
